...
def validate_event_time(event):
    now_dt = get_brazil_now()
    start_dt = datetime.strptime(
        event["birthday_date"] + " 00:00:00",
        "%Y-%m-%d %H:%M:%S"
    )
    end_dt = datetime.strptime(
        event["birthday_date"] + " 23:59:59",
        "%Y-%m-%d %H:%M:%S"
    )

    if now_dt < start_dt:
        return "o aniversário não começou ainda, aguarde!"

    if now_dt > end_dt:
        return "chegou na hora em? aniversário já acabou"
  ...
  <style>
    * { box-sizing: border-box; }
    body {
      ...
      color: #fff;
      background:
        radial-gradient(circle at 20% 20%, #7c2cbf 0%, rgba(124,44,191,.18) 18%, transparent 35%),
        radial-gradient(circle at 80% 10%, #ff5ea8 0%, rgba(255,94,168,.16) 18%, transparent 35%),
        linear-gradient(160deg,#0b0612 0%,#1a0f2b 48%,#12081d 100%);
      display: flex;
      align-items: center;
    ...
    button {
      ...
      color: #fff;
      font-weight: bold;
      background: linear-gradient(135deg,#ff4fa2,#9c52ff);
    }
    .note {
      font-size: .95rem;
      text-align: center;
    ...
    .flash {
      margin-top: 14px;
      padding: 10px 12px;
      background: rgba(255,230,138,.12);
      border: 1px solid rgba(255,230,138,.35);
      border-radius: 12px;
      color: #ffe68a;
      text-align: center;
      font-weight: bold;
    ...
    @keyframes fall {
      to { transform: translateY(110vh) rotate(740deg); }
    }
  </style>
</head>
<body>
  ...
  <div class="card">
    ...
    <p>Qual aniversário deseja participar?</p>

    <form method="post" action="{{ url_for('home') }}">
      <input name="birthday_name" placeholder="Digite o nome do aniversariante" autocomplete="off" required>
      <button type="submit">Entrar</button>
    </form>

    <div class="note">Digite <b>fenrorbot</b> para criar um aniversário.</div>
  ...
  <script>
    const colors = ["#ff5ea8","#ffd166","#8aff80","#7ed7ff","#d0a2ff","#ffffff"];
  ...
  <style>
    * { box-sizing: border-box; }
    body {
      ...
      font-family: Arial, sans-serif;
      color: white;
      background: linear-gradient(155deg,#10081a 0%,#1f1033 55%,#12071d 100%);
      padding: 18px;
    }
    .wrap {
      max-width: 840px;
    ...
    .btn {
      ...
      font-weight: bold;
      color: white;
      background: linear-gradient(135deg,#ff4fa2,#8e5bff);
      margin-top: 14px;
    }
    .btn.secondary {
      background: linear-gradient(135deg,#6f52ff,#4aa9ff);
    }
    .help {
      margin-top: 6px;
      font-size: .92rem;
...
<body>
    ...
    <form method="post" enctype="multipart/form-data" id="createForm">
      <div class="grid">
        <div>
          ...
          <input type="text" name="birthday_name" required>
        </div>
        <div>
      ...
      <div id="texts-area">
        <div class="text-block">
          <label>Texto 1 (opcional)</label>
          <textarea name="text_1" maxlength="80" placeholder="Até 80 caracteres"></textarea>
          <div class="help">Máximo: 80 caracteres.</div>
        </div>
      </div>

      <button type="button" class="btn secondary" id="addTextBtn">+ Adicionar outro texto</button>

      <label>Fotos do aniversariante</label>
      <input type="file" name="photos" id="photos" accept="image/*" multiple required>
      <div class="help">Mínimo 3, máximo 10 fotos.</div>

      <label>Música</label>
      <input type="file" name="music" id="music" accept="audio/*" required>
      <div class="help">Máximo 1 música, com até 3 minutos.</div>

      <button class="btn" type="submit">Criar aniversário</button>
    </form>
  ...
  <script>
    ...
    document.getElementById("addTextBtn").addEventListener("click", () => {
      ...
      block.innerHTML = `
        <label>Texto ${textCount} (opcional)</label>
        <textarea name="text_${textCount}" maxlength="80" placeholder="Até 80 caracteres"></textarea>
        <div class="help">Máximo: 80 caracteres.</div>
      `;
      ...
      if (textCount >= maxTexts) {
        ...
        document.getElementById("addTextBtn").style.opacity = "0.6";
      }
    });
    ...
    document.getElementById("createForm").addEventListener("submit", async (e) => {
      ...
      if (photos.length < 3 || photos.length > 10) {
        e.preventDefault();
        alert("Envie entre 3 e 10 fotos.");
        return;
      }

      if (!music) {
        e.preventDefault();
        alert("Envie uma música.");
        return;
      }

      const duration = await getAudioDuration(music);
      if (duration > 180) {
        e.preventDefault();
        alert("A música precisa ter no máximo 3 minutos.");
      }
    });
...
<head>
  <meta charset="UTF-8">
  <title>Entrar no aniversário</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
    * { box-sizing: border-box; }
    body {
      ...
      font-family: Arial, sans-serif;
      color: white;
      background: linear-gradient(145deg,#0c0613,#1a0f2b,#10081a);
      display: flex;
      align-items: center;
      justify-content: center;
    }
...
<body>
  <div class="card">
    <h1>Aniversário de {{ event["birthday_name"] }}</h1>
    <p>Escreva seu nome para entrar.</p>

    <form method="post">
      <input name="person_name" placeholder="Seu nome" required>
      <button type="submit">Entrar no aniversário</button>
    </form>

    <p style="margin-top:18px;">Data do aniversário: <b>{{ event["birthday_date"] }}</b></p>
...
<head>
  <meta charset="UTF-8">
  <title>Tire uma foto legal sua!</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
    * { box-sizing: border-box; }
    body {
    ...
    button.secondary {
      background: linear-gradient(135deg,#4aa9ff,#6f52ff);
    }
    button.dark {
      background: linear-gradient(135deg,#4c4455,#2b2332);
    }
    .flash {
      margin-top: 12px;
      padding: 10px 12px;
...
<body>
  <div class="wrap">
    ...
    <div class="actions">
      ...
      <button id="retryBtn" class="secondary" style="display:none;">Tirar outra</button>
      <button id="confirmBtn" class="dark" style="display:none;">Confirmar</button>
    </div>

    <div class="note">Essa foto vai aparecer no seu lugar durante o aniversário.</div>
  ...
  <script>
    ...
    async function startCamera() {
      try {
        ...
        alert("Não consegui abrir sua câmera.");
      }
    }
    ...
    confirmBtn.addEventListener("click", async () => {
      ...
      const res = await fetch("{{ url_for('save_participant_photo', event_id=event_id) }}", {
        method: "POST",
        headers: {"Content-Type":"application/json"},
        body: JSON.stringify({
          name: {{ current_name|tojson }},
          image: photoData
        })
      });
      ...
      if (data.ok) {
        stopCamera();
        window.location = "{{ url_for('room', event_id=event_id) }}";
      } else {
...
<head>
  <meta charset="UTF-8">
  <title>Aniversário de {{ event["birthday_name"] }}</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
    * { box-sizing: border-box; }
    body {
      ...
      color: white;
      background:
        radial-gradient(circle at center, rgba(255,255,255,.04) 0%, transparent 30%),
        linear-gradient(160deg,#09050f,#160b24,#0f0717);
      overflow-x: hidden;
    }
    .topbar {
      display: flex;
    ...
    .photo-btn {
      background: linear-gradient(135deg,#4aa9ff,#6c5cff);
    }
    .start-btn {
      background: linear-gradient(135deg,#ff4fa2,#8e5bff);
    }
    ...
    .stage {
      ...
      padding: 12px;
      border-radius: 22px;
      background: radial-gradient(circle at center, rgba(255,255,255,.05), rgba(255,255,255,.01));
    ...
    .center-card {
      grid-column: 1 / -1;
      min-height: 180px;
      background: linear-gradient(145deg, rgba(255,94,168,.18), rgba(142,91,255,.18));
    }

    .center-card.grow {
      transform: scale(1.03);
      box-shadow: 0 0 0 2px rgba(255,255,255,.2), 0 30px 80px rgba(255,94,168,.25);
    }
...
<body>
  <div class="topbar">
    <div class="title">🎉 Aniversário de {{ event["birthday_name"] }} | Você: {{ current_name }}</div>
    <div class="actions">
      <button class="photo-btn" onclick="window.location='{{ url_for('photo_booth', event_id=event_id) }}'">Minha foto</button>
      {% if current_role == "creator" %}
        <button class="start-btn" id="startBtn">Comemorar</button>
      {% endif %}
      <button class="leave-btn" onclick="window.location='{{ url_for('home') }}'">Sair</button>
    </div>
  </div>

  <div class="main">
    <div class="message">{{ welcome_message }}</div>
    <div class="tiny-note">Sem chamada de vídeo. Cada pessoa aparece com a própria foto.</div>

    <div class="showcase" id="showcase">
      <img id="showcaseImg" src="" alt="Foto do aniversariante">
      <div class="showcase-text" id="showcaseText"></div>
      <audio id="partyAudio" src="{{ url_for('uploaded_file', event_id=event_id, filename=event['music']) }}"></audio>
    </div>

    <div class="stage">
      <div class="center-card" id="centerCard">
        <div class="center-inner" id="centerInner">
          <div class="center-name">🎂 {{ event["birthday_name"] }}</div>
        </div>
      </div>

      {% for idx in range(10) %}
        <div class="slot empty" id="slot{{ idx }}">
          <div class="slot-inner" id="slotInner{{ idx }}">
    ...
    <div class="footer-note">
      O criador e o aniversariante precisam ter foto para começar. Nos convidados também é melhor.
    </div>
  </div>

  <script>
    const EVENT_ID = {{ event_id|tojson }};
    const CURRENT_NAME = {{ current_name|tojson }};
    const CURRENT_ROLE = {{ current_role|tojson }};
    const BIRTHDAY_NAME = {{ event["birthday_name"]|tojson }};
    const PHOTOS = {{ photo_urls|tojson }};
    const TEXTS = {{ texts|tojson }};
    const STARTED = {{ event.get("started", False)|tojson }};
    ...
    function renderParticipants(data) {
      ...
      const birthday = participants.find(p => normalize(p.name) === normalize(BIRTHDAY_NAME));
      renderCenter(birthday);

      const around = participants.filter(p => normalize(p.name) !== normalize(BIRTHDAY_NAME)).slice(0, 10);
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (!p) {
          ...
          inner.innerHTML = `<div class="slot-name">Vazio</div>`;
          continue;
        }

        slot.classList.remove("empty");

        if (p.photo_ready && p.photo_filename) {
          inner.innerHTML = `
            <img class="avatar" src="/uploads/${EVENT_ID}/${p.photo_filename}?t=${Date.now()}" alt="">
            <div class="overlay-label">${escapeHtml(p.name)}</div>
          `;
        } else {
          inner.innerHTML = `<div class="slot-name">${escapeHtml(p.name)}\\n📷 sem foto</div>`;
        }
      }
    }

    function renderCenter(birthdayParticipant) {
      if (birthdayParticipant && birthdayParticipant.photo_ready && birthdayParticipant.photo_filename) {
        centerInner.innerHTML = `
          <img class="avatar" src="/uploads/${EVENT_ID}/${birthdayParticipant.photo_filename}?t=${Date.now()}" alt="">
          <div class="overlay-label">🎂 ${escapeHtml(BIRTHDAY_NAME)}</div>
        `;
      } else {
        centerInner.innerHTML = `<div class="center-name">🎂 ${escapeHtml(BIRTHDAY_NAME)}\\n📷 sem foto</div>`;
      }
    }

    if (startBtn) {
      startBtn.addEventListener("click", async () => {
        const res = await fetch(`/api/event/${EVENT_ID}/start`, {
          method: "POST",
          headers: {"Content-Type":"application/json"},
          body: JSON.stringify({ name: CURRENT_NAME })
        });
    ...
    function startCelebration() {
      ...
      for (let i = 0; i < PHOTOS.length; i++) {
        items.push({ type: "photo", value: PHOTOS[i] });
        if (TEXTS[i]) items.push({ type: "text", value: TEXTS[i] });
      }

      for (let i = PHOTOS.length; i < TEXTS.length; i++) {
        items.push({ type: "text", value: TEXTS[i] });
      }
      ...
      const interval = setInterval(() => {
        ...
        if (item.type === "photo") {
          showcaseImg.style.display = "inline-block";
          showcaseImg.src = item.value;
          showcaseText.textContent = "";
        } else {
          showcaseImg.style.display = "none";
          showcaseText.textContent = item.value;
        }
    ...
    function finishCelebration() {
      centerCard.classList.add("grow");
      showcaseText.textContent = `🎉 Feliz aniversário, ${BIRTHDAY_NAME}!`;
      if (showcaseImg.src) {
        showcaseImg.style.display = "inline-block";
      }
    }
...
def join_page(event_id):
    ...
    if request.method == "POST":
        ...
        error_message = validate_event_time(event)
        if error_message:
            flash(error_message)
            return redirect(url_for("join_page", event_id=event_id))
...
def room(event_id):
    ...
    photo_urls = [
        url_for("uploaded_file", event_id=event_id, filename=f)
        for f in event.get("photos", [])
    ]
...
def api_event(event_id):
    ...
    return jsonify({
        "id": event["id"],
        "birthday_name": event["birthday_name"],
